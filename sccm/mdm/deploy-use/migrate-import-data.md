...
2.  インポートのために選択できるオブジェクトに関する詳細と、一部のオブジェクトがインポートできない理由に関する情報を提供する。
...
- 他のオブジェクトに依存しているいくつかのプロファイルがあります。 証明書に依存している電子メール プロファイルのように、別のオブジェクトに依存しているプロファイルをインポートする場合は、同時に両方のオブジェクトをインポートする必要があります。
...
- SMS プロバイダー (Configuration Manager データを収集するため) とインターネット (Intune にオブジェクトをインポートするため) にアクセスできるコンピューターからツールを実行する必要があります。
...
1. 「[Intune Data Importer GitHub](https://go.microsoft.com/fwlink/?linkid=858194)」ページに移動します。
2. **[Clone or download]\(複製またはダウンロード\)** をクリックして、**[Download ZIP]\(ZIP のダウンロード\)** をクリックして圧縮された ZIP ファイルを保存します。 
3. ZIP ファイルの内容を抽出します。
...
2. 情報を読み、**[次へ]** をクリックします。 
3. サイトおよびインポートするサイトのオブジェクトに関する次の情報を指定します。 
...
5.  **[次へ]** をクリックして、収集されたオブジェクトを確認します。 項目の選択ページは、収集されたオブジェクトの種類ごとに使用できます。 
    ...
    > [!Tip]     
    > 各項目の選択ページでは、インポートするオブジェクトを見つけやすくするためのフィルターを作成できます。 ただし、次の点に注意してください。
    > - 項目の選択ページでビューがフィルター処理されている場合、すべてのチェックボックスの選択は、表示されている項目のみに適用されます。 フィルターにより非表示になっているオブジェクトは、チェックボックスを使用する場合には含まれません。
    > - オブジェクトは、並べ替えまたはフィルターを適用した場合でも、常にその親項目の下にグループ化されます。
...
8.  項目の選択の各ページで、インポートするオブジェクトを選択します。 次の列が一覧表示されます。
    ...
    - **[Notes]\(備考\)**: オブジェクトをインポートできない理由に関する情報を提供します。
...
### <a name="phase-3-import-selected-object-to-intune"></a>フェーズ 3: 選択したオブジェクトを Intune にインポートする
フェーズ 3 では、Intune にサインインして、選択したオブジェクトをインポートします。 
10. **[次へ]** をクリックし、**[Sign in to Intune]\(Intune にサインインする\)** をクリックして、データ インポートの Intune テナントにサインインします。 グローバル管理者アカウントまたは Intune 管理者アカウントでサインインする必要があります。 サインインすると、インポート プロセスが開始します。
