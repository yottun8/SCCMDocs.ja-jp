...
##  <a name="child-task-sequence"></a>子タスク シーケンス

Configuration Manager バージョン 1710 以降では、別のタスク シーケンスを実行する新しいタスク シーケンス ステップを追加できます。 これにより、タスク シーケンス間に親子のリレーションシップが作成されます。 子タスク シーケンスを使用すると、モジュール型の再利用可能なタスク シーケンスを作成できます。
...
> [!Note]  
>  クライアントは、**参照オペレーティング システム イメージを構築およびキャプチャする** タスク シーケンスでのみ削除されます。 メディアのキャプチャやカスタムのタスク シーケンスなど、その他のキャプチャ メソッドの場合、クライアントは削除されません。
