...
-   **サービス プランの監視のタイル**: 作成したサービス プランとそれぞれに対するコンプライアンスのグラフが表示されます。 このタイルにより、サービス プランの展開の現在の状態の概要をすばやく確認できます。 前の展開リングがコンプライアンスに対する期待を満たす場合は、サービス プラン ルールが自動的にトリガーされることを待たずに、後のサービス プラン (展開リング) を選択し、 **[今すぐ展開]** をクリックすることができます。  
...
-   **時間遅延**: サービス プランで、 **[Microsoft が新しいアップグレードを公開してから何日後に環境に展開されるようにするか]** として指定する日数です。 現在の日付が、リリース日に構成された日数を加算した日付よりも後である場合、Configuration Manager はアップグレードを展開に含めるかどうかを評価します。  
