...
<!-- 1357126 --> 1 つのインストールが完了するのを待ってから次のインストールを開始するのではなく、一度に複数のアプリケーションをインストールします。 以下はすべてのアプリケーションに当てはまるわけではありません。
- アプリがユーザーに表示される
...
- **次の日時後にインストールするようにスケジュール済み**: デバイスに近日公開されているソフトウェアがインストールされる次のメンテナンス期間の日時が示されます。 IT 管理者がメンテナンス期間を定義します。<!--1358131-->
    ...
    - **[今すぐインストール]** ボタンをクリックすることで、メンテナンス期間になる前にインストールできます。 

IT 管理者により、ユーザーがアプリケーション カタログ Web サイトのアプリケーションを見ることが許可されている場合があります。 Web サイトを表示するには、右上隅の **[アプリケーション カタログ Web サイトを開く]** をクリックします。 <!--1358214-->
