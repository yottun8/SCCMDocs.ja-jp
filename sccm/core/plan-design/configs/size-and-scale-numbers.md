---
title: "サイズとスケール | System Center Configuration Manager"
...
この記事のこれらのサポートの数は、Configuration Manager で推奨されるハードウェアを使用している場合に基づいています。 推奨されるハードウェアを使用しない場合は、サイト システムのパフォーマンスが低下し、規定されたレベルのサポートを満たさない可能性があります。
...
-   サイト サーバーから離れた場所にあるソフトウェアの更新ポイントで、最大 150,000 台のクライアントをサポートできます。ただし、このクライアント数をサポートするには、リモート コンピューターが WSUS の要件を満たしている必要があります。  
...
-   展開でモバイル デバイス管理をサポートする方法に応じた、次のいずれか:  
    ...
    -   オンプレミス MDM で管理する 100,000 個のデバイス  

    -   300,000 個のクラウドベースのデバイス  

 たとえば、階層では、700,000 個のデスクトップ、最大 25,000 個の Mac および Windows CE 7.0 クライアント、および Microsoft Intune と統合する場合は最大 300,000 個のクラウドベースのデバイスをサポートでき、合計 1,025,000 個のデバイスになります。  オンプレミス MDM で管理されているデバイスをサポートする場合は、階層のデバイス数の合計は 825,000 個です。  
...
-   特定のグループまたは種類に制限されない合計 150,000 個のクライアントとデバイス (サポートが階層のサポートされている数を超えない場合)  

たとえば、(階層の制限があるため) Mac と Windows CE 7.0 を実行する 25,000 コンピューターをサポートするプライマリ サイトは、追加の 125,000 デスクトップ コンピューターをサポートできます。これによって、サポートされるデバイスの合計数の上限は、子のプライマリ サイトでサポートされる 150,000 の上限まで上がります。
...
-   次の制限を持つ合計 175,000 個のクライアントとデバイス:  
    ...
    -   展開でモバイル デバイス管理をサポートする方法に応じた、次のいずれか:  
        ...
        -   オンプレミス MDM で管理する 50,000 個のデバイス  

        -   150,000 個のクラウドベースのデバイス  

たとえば、150,000 個のデスクトップと 10,000 個の Mac または Windows CE 7.0 クライアントをサポートするスタンドアロン プライマリ サイトでは、追加のデバイスを 15,000 個のみサポートできます。 これらのデバイスは、クラウドベースにすることも、オンプレミス MDM を使用して管理することもできます。  
...
-   次の制限を持つ合計 25,000 個のクライアントとデバイス:  
    ...
    -   次のいずれか (両方ではありません):  
        ...
        -   オンプレミス MDM を使用して管理される 10,000 個のデバイス  

        -   Mac および Windows CE 7.0 を実行している 10,000 個のデバイス  
