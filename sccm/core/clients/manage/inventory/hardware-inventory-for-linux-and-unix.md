...
###  <a name="BKMK_LinuxProvider"></a> Linux および UNIX コンピューターのカスタム ハードウェア インベントリを作成する:  
 ...
 OMI ソース- v.1.0.6 およびソース OMI 入門ガイドはどちらも「 [Open Group](http://go.microsoft.com/fwlink/p/?LinkId=262317) 」Web サイトからダウンロードできます。 ダウンロードの場所は、OpenGroup.org の Web サイトの Web ページの **[Documents]** タブで見つけることができます ([Open Management Infrastructure (OMI)](http://go.microsoft.com/fwlink/p/?LinkId=286805))。  
...
###  <a name="BKMK_AddLinuxProvidertoCM"></a> Configuration Manager で、新しいインベントリ クラスを次のように有効にします。  
 Configuration Manager は、Linux および UNIX のコンピューターで、新しいプロバイダーによって報告されるインベントリをレポートできます、前に、カスタム プロバイダーのスキーマを定義するマネージド オブジェクト フォーマット (MOF) ファイルをインポートする必要があります。  
