...
バージョン 1610 以降、**クライアント データ ソース** ダッシュボードを使用して、環境内の[ピア キャッシュ](/sccm/core/plan-design/hierarchy/client-peer-cache)の使用状況を把握できるようになりました。 コンソールでこのダッシュボードを表示するには、ピア キャッシュを使用してコンテンツをダウンロードし、その情報をサイトに報告する必要があります。 レポート間隔があるので、この処理に 24 時間かかることがあります。
