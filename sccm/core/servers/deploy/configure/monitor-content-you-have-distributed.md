...
> [!TIP]  
> バージョン 1610 では、ピア キャッシュとクライアント データ ソースのダッシュボードは、プレリリース機能です。 有効にするには、「[更新プログラムからのプレリリース機能の使用](/sccm/core/servers/manage/install-in-console-updates#bkmk_prerelease)」をご覧ください。 **[クライアント データ ソース]** ダッシュボードは、有効にしないと表示されません。 有効にしてからデータの表示を開始するまで、最大で 24 時間かかる場合があります。 
