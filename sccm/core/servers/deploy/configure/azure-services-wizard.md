...
### <a name="renew-secret-key-azure-ad-apps"></a>Azure AD アプリの秘密鍵を更新する
1806 より前のバージョンでは、Azure アプリの秘密鍵を更新するにはアプリを再作成する必要があります。

バージョン 1806 以降:

- 作成されたアプリの場合:**Cloud Services ノード**の下で **Azure Active Directory テナント**に移動します。 詳細ウィンドウで、アプリケーションが作成されたテナントを選択し、**[秘密鍵を更新する]** を選択します。  

    - **[サインイン]** を選択して、管理ユーザーとして Azure への認証を行います。  

    - **[OK]** を選択して Azure AD でネイティブ アプリを作成し、クライアント アプリケーションの作成ダイアログを閉じます。 この操作で、[クライアント アプリのダイアログ](#client-app-dialog)に戻ります。  

- インポートされたアプリの場合:Azure portal を使って更新し、新しい秘密鍵と有効期限日をメモします。 **[秘密鍵を更新する]** ウィザード上にこの情報を追加します。  
...
-   **Log Analytics コネクタ** サービス、**[構成]** ページ:[Log Analytics への接続を構成する](/sccm/core/clients/manage/sync-data-log-analytics#configure-the-connection-to-log-analytics)  
...
-   **ビジネス向け Microsoft Store** サービス、**[構成]** ページ:[ビジネス向け Microsoft Store の同期を構成する](/sccm/apps/deploy-use/manage-apps-from-the-windows-store-for-business#bkmk_config)  
