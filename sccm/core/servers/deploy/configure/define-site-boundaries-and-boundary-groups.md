...
クライアントから要求されているコンテンツが現在の境界グループ内の有効なコンテンツ ソースから利用できない場合、クライアントは近隣の境界グループまたはサイトの既定の境界グループに対するフォールバック期間に達するまで待機してから、追加のコンテンツ ソースを検索できるようになります。

コンテンツがオンデマンドで分散されておらず、クライアントからの要求時に使用できない場合、現在の境界でコンテンツを配布ポイントに転送するプロセスが開始されます。  
...
##  <a name="BKMK_BoundaryBestPractices"></a> 境界について推奨する運用方法  
