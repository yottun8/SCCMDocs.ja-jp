...
**期限切れの通知タスク履歴の削除**: このタスクは、クライアント通知タスクに関する情報が、指定した時間更新されなかったときに、サイト データベースから情報を削除する場合に使用します。 クライアント通知の詳細については、「[System Center Configuration Manager のクライアント展開タスク](../../../core/clients/deploy/client-deployment-tasks.md)」を参照してください  
